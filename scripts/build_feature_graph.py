...
def determine_function_subcategory(name):
    ...
    if any(
        ...
        k in name_lower for k in ["length", "nodes", "relationships"]
    ) and "path" not in name_lower.replace("length", ""):
        return "path"
    else:
        return "other"
...
def build_graph():
    ...
    print(f"  Features: {result[0]['count'].value if result else 0}")

    result = gf.execute("MATCH (n:Category) RETURN count(n) AS count")
    print(f"  Categories: {result[0]['count'].value if result else 0}")

    result = gf.execute("MATCH (n:Implementation) RETURN count(n) AS count")
    print(f"  Implementations: {result[0]['count'].value if result else 0}")

    print("\nRelationship Statistics:")
    result = gf.execute("MATCH ()-[r:IMPLEMENTED_IN]->() RETURN count(r) AS count")
    print(f"  IMPLEMENTED_IN: {result[0]['count'].value if result else 0}")

    result = gf.execute("MATCH ()-[r:BELONGS_TO_CATEGORY]->() RETURN count(r) AS count")
    print(f"  BELONGS_TO_CATEGORY: {result[0]['count'].value if result else 0}")
    ...
    sorted_result = sorted(result, key=lambda r: r["count"].value, reverse=True)
    ...
    sorted_result = sorted(result, key=lambda r: r["count"].value, reverse=True)[:10]
    for row in sorted_result:
