...
def main():
    ...
    output.append("Comprehensive inventory of all OpenCypher Technology Compatibility Kit (TCK) test scenarios in GraphForge.\n")
    ...
    for category in sorted(categories.keys()):
        ...
        output.append(f"- [{category}](#{category.replace('/', '-')}) ({len(categories[category])} files, {count} scenarios)\n")
    ...
    output.append(f"| **TOTAL** | **{len(feature_files)}** | **{total_scenarios}** | **{total_scenarios/len(feature_files):.1f}** |\n")
        ...
        for file_info in sorted(files, key=lambda x: x['file']):
            ...
            if file_info['scenarios']:
                ...
                for i, scenario in enumerate(file_info['scenarios'], 1):
                    output.append(f"{i}. {scenario['name']} ({scenario['type']}, line {scenario['line']})\n")
